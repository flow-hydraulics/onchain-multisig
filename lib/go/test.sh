...
if [ ! -f "./flow.json" ]; then
  echo "IMPORTANT: This script must be run from the 'onchain-multisig' root folder, not a subdirectory"
  exit 1
fi
...
if [ "${NETWORK}" == "emulator" ]; then
  ...
  # the first account is the owner of the OnChainMultiSig contract
